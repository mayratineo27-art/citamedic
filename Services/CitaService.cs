...
namespace PostaCitasWeb.Services
{
    ...
    public class CitaService : ICitaService
    {
        ...
        public async Task<bool> CreateCitaAsync(Cita cita)
        {
            ...
            if (cita.FechaReserva.Date == DateTime.Today)
            {
                return false;
            }

            if (DateTime.Today.DayOfWeek == DayOfWeek.Saturday && cita.FechaReserva.Date == DateTime.Today.AddDays(2))
            {
                return false;
            }
        ...
        public async Task<CitaResult> ReserveCitaAsync(int pacienteId, int slotId, OrigenReserva origen, int? registradoPorUsuarioId = null)
        {
            try
            {
                ...
                if (slot == null)
                    return CitaResult.CreateFailure("El slot no existe.");

                if (slot.CuposDisponibles <= 0)
                    return CitaResult.CreateFailure("No hay cupos disponibles en este horario.");

                // RN31: Verificar que no exista otra cita activa para el mismo paciente y slot
                bool tieneActivaEnSlot = await _citaRepository.HasActiveCitaInSlotAsync(pacienteId, slotId);
                if (tieneActivaEnSlot)
                    return CitaResult.CreateFailure("Ya tienes una cita reservada en este horario.");
        ...
        public async Task<CitaResult> CancelCitaAsync(int citaId, string motivo = "")
        {
            try
            {
                ...
                var slot = await _slotRepository.GetByIdAsync(cita.SlotId);
                if (slot == null)
                ...
                if (DateTime.UtcNow.TimeOfDay > horaLimite.ToTimeSpan())
