...
namespace PostaCitasWeb.Services
{
    ...
    public class AuthService : IAuthService
    {
        ...
        public async Task<AuthResult> HabilitarUsuarioAsync(int usuarioId, int admisionUsuarioId)
        {
            ...
            if (solicitante == null || solicitante.Rol != Rol.Admision)
        ...
        public async Task<string> SolicitarRecuperacionAsync(string dni, string celular)
        {
            ...
            if (usuario.Celular != celular)
                throw new ValidationException("El número celular no coincide con el registrado");

            string token = Guid.NewGuid().ToString();
            return token;
        }

        public async Task<AuthResult> ActualizarDatosAsync(int usuarioId, ActualizarDatosDto dto)
        {
            var usuario = await _usuarioRepository.GetByIdAsync(usuarioId);
            if (usuario == null)
                return AuthResult.CreateFailure("Usuario no encontrado.");

            if (dto.DNI != null || dto.Nombres != null || dto.ApellidoPaterno != null || 
                dto.ApellidoMaterno != null || dto.FechaNacimiento != null)
            {
                throw new InvalidOperationException("No está permitido modificar este campo");
            }
            ...
            return AuthResult.CreateSuccess(usuario.UsuarioId, usuario.NombreUsuario, null, usuario.Rol.ToString());
        }
